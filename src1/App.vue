<template>
  <div id="app">
    ...
    <main>
      ...
      <!-- 歌单详情页面：需同时满足“当前页面是playlist”且“有选中的歌单”  -->
      <PlaylistPage 
...
export default {
  ...
  setup() {
    ...
    // 4. 用户数据状态：收藏列表、最近播放、播放次数统计
    const favorites = ref([1, 3, 5])// 已收藏歌曲的id列表
    const recentlyPlayed = ref([]) // 最近播放列表
    const playCounts = ref({})// 歌曲播放次数统计
    const lastPlayedSong = ref(null) // 最后播放的歌曲
    
   // 5. 筛选与搜索状态：音乐库页面的筛选条件
    const currentGenre = ref('all')// 当前选中的歌曲风格（默认“全部”）
    const searchQuery = ref('')// 搜索查询
    
    // 6. 歌单状态：当前查看的歌单及其中的歌曲
    const currentPlaylist = ref(null)// 当前选中的歌单对象
    const playedInRandomMode = ref([])// 用于随机播放的已播歌曲记录
    ...
    const songs = ref([
      ...
      },
      {
        ...
        audioUrl: 'audio/起风了-买辣椒也用券-标准.mp3'
      },
      {
        id: 6,
        name: '夜空中最亮的星',
        ...
        audioUrl: 'audio/夜空中最亮的星-逃跑计划-标准.mp3'
      },
      {
        id: 7,
        name: '卡农',
      ...
      },
      {
        ...
        audioUrl: 'audio/月光奏鸣曲-贝多芬-标准.mp3'
      },
      {
        id: 9,
        name: '海阔天空',
